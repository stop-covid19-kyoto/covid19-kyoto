...
######
# 京都府版のi18n関連ファイルのうち、元となるja.jsonを自動生成するスクリプト。
# 東京都版の翻訳と京都府版の翻訳を両方使えるようにして、保守性を保つことを目的としています。
# このスクリプトは兵庫県版で使われているスクリプトを京都府版向けに改変したものです。
######
...
CHECK_RESULT = 'result_kyoto.csv'

# 東京都版の翻訳ファイルパス
TOKYO_JA_JSON_PATH = os.path.join(os.pardir, "assets/locales/ja.json")

# 京都府版の翻訳ファイルパス
KYOTO_JA_JSON_PATH = os.path.join(os.pardir, "assets/locales-kyoto/ja.json")
...
with open(os.path.join(os.pardir, OUTPUT_DIR, CHECK_RESULT), mode="a", encoding=ENCODING) as result, \
        ...
        open(KYOTO_JA_JSON_PATH, mode='r', encoding=ENCODING) as kyoto_file:
    # ディレクトリ毎にテスト
    ...
    # 東京都版と京都府版のjsonをそれぞれ読み込む
    tokyo_json = json.load(tokyo_file)
    kyoto_json = json.load(kyoto_file)
    ...
    kyoto_json_keys = list(kyoto_json.keys())

    # 以前はなかったが今はある翻訳を追加する
    for key in tentative_json_keys:
        kyoto_tag = kyoto_json.get(key)
        tentative_tag = tentative_ja_json.get(key)
        if not kyoto_tag:
            kyoto_json[key] = tentative_tag
            print("Add TAG: " + str(tentative_tag) + " to " + KYOTO_JA_JSON_PATH)
            if not warn_count:
                result.write(",".join(["RUN", datetime.now(jst).strftime("%Y/%m/%d %H:%M")]) + '\n')
            result.write(",".join(["TAG_ADD", str(tentative_tag)]) + '\n')
            warn_count += 1
        elif kyoto_tag != tentative_tag and isinstance(kyoto_tag, dict):
            tentative_dict_keys = tentative_tag.keys()
            for dict_key in tentative_dict_keys:
                if not kyoto_tag.get(dict_key):
                    kyoto_tag[dict_key] = tentative_tag[dict_key]
                    print("Add TAG: " + str(tentative_tag[dict_key]) + " to " + KYOTO_JA_JSON_PATH)
                    if not warn_count:
                        result.write(",".join(["RUN", datetime.now(jst).strftime("%Y/%m/%d %H:%M")]) + '\n')
                    result.write(",".join(["TAG_ADD", str(tentative_tag[dict_key])]) + '\n')
        if kyoto_tag:
            kyoto_json_keys.pop(kyoto_json_keys.index(key))

    # 以前はあったが今はない翻訳を削除する
    for key in kyoto_json_keys:
        ja_tag = kyoto_json.get(key)
        kyoto_json.pop(key)
        print("Remove TAG: " + str(ja_tag) + " from " + KYOTO_JA_JSON_PATH)
        if not warn_count:
            result.write(",".join(["RUN", datetime.now(jst).strftime("%Y/%m/%d %H:%M")]) + '\n')
        result.write(",".join(["TAG_REMOVE", str(ja_tag)]) + '\n')
        warn_count += 1

    made_json = kyoto_json

with open(KYOTO_JA_JSON_PATH, mode="w", encoding=ENCODING) as kyoto_file:
    json.dump(made_json, kyoto_file, ensure_ascii=False, indent=2)
